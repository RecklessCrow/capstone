...
class RetroEnv(gym.Env):
    ...
    def __init__(self, game, state=retro.State.DEFAULT, scenario=None, info=None,
        ...
        if self.gamename == 'Galaga-Nes':
            self.old_stage = self.data.lookup_value("current_stage")
            self.old_lives = 2
    ...
    def reset(self):
        ...
        if self.movie_path is not None:
            ...
            self.record_movie(
                os.path.join(self.movie_path, './recordings/%s-%s-%06d.bk2' % (self.gamename, rel_statename, self.movie_id)))
            self.movie_id += 1
        if self.movie:
            self.movie.step()
        self.data.reset()
        ...
        if self.gamename == 'Galaga-Nes':
            self.old_stage = self.data.lookup_value("current_stage")
            self.old_score = self.data.lookup_value("score")
            self.old_lives = 2
        elif self.gamename == 'DigDug-Nes':
            self.old_lives = 3
    ...
    def get_galaga_reward(self):
        ...
        # Negative reward if ship is hit
        if current_lives < self.old_lives:
            self.old_lives -= 1
            return -1
        elif current_lives > self.old_lives:
            self.old_lives += 1
            reward = 1

        # Reward hitting a ship
        if current_score > self.old_score:
            # Divide score by 10000 due to reward being clipped to -1 and 1.
            # This gives the model more grainulairty to its rewards
            reward = (current_score - self.old_score) / 10000
            self.old_score = current_score

        # Positive reward if ship gets to next level
        if current_stage > self.old_stage:
            self.old_stage = current_stage
            reward += 1
